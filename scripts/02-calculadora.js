...
const calculate = () => {
    ...
    const number1 = input1 ? +input1.value : NaN;
    // let number1: number;
    ...
    const number2 = input2 ? +input2.value : NaN;
    ...
    const text = document.getElementById("blockResult");
    if (selectedOperator != "") {
        // if (!isNaN(number1) || !isNaN(number2)) {
        if (number1 != 0 || number2 != 0) {
            switch (selectedOperator) {
                case "addition":
                    accumulatedResult += `${number1} + ${number2} = ${number1 + number2}\n`;
                    break;
                case "subtraction":
                    accumulatedResult += `${number1} - ${number2} = ${number1 - number2}\n`;
                    break;
                ...
                case "exponent":
                    ...
                    break;
                case "modulus":
                    accumulatedResult += `Residuo de (${number1}/${number2}) es ${number1 % number2}\n`;
                    break;
                // default:
                //   break;
            }
