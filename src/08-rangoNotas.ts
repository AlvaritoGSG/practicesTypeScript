...
const renderOptionsSelect = () => {
  const inputSelectNombre = document.getElementById(
    "nombresInput",
...
const renderNotesList = () => {
  const notasList = document.querySelector("ul") as HTMLUListElement | null;
  if (notasList !== null) {
    studentData.forEach((estudiante) => {
      if (estudiante.nota !== null) {
...
const loadNotasFromStorage = (): boolean => {
  try {
    const dataCruda = localStorage.getItem("notas");
    if (!dataCruda) return false;
    if (typeof dataCruda !== "string") return false;
    ...
    console.warn(
      "localStorage: datos corruptos o JSON inválido, se restablece.",
      error,
    );
    try {
...
const resetStudentData = async () => {
  try {
    ...
    console.warn(
      "Error al cargar notas. Se restablecio lista por defecto.",
      error,
    );
  } finally {
    // localStorage.setItem("notas", JSON.stringify(studentData));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const notaInput = document.getElementById("notaInput") as HTMLInputElement;
  const verRangoBtn = document.getElementById(
    "verRangoBtn",
  ) as HTMLButtonElement;
  const notasList = document.querySelector("ul") as HTMLUListElement;

  const inputValidation = (input: HTMLInputElement): boolean => {
    const nota = Number(input.value);
    const parentInput = input.parentElement as HTMLElement;
    const smallHelper = parentInput.querySelector("small") as HTMLElement;
