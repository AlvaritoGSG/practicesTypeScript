...
const getFibonacciSeries = () => {
    const fibonacciQuantity = parseInt(document.getElementById("fibonacciQuantity").value);
    const fibonacciResult = document.getElementById("fibonacciResult");
    fibonacciResult.innerHTML = "";
    const fibonacciSeries = [];
    for (let index = 1; index <= fibonacciQuantity; index++) {
        fibonacciSeries.push(index === 1
            ? 0
            : index === 2
                ? 1
...
const getFactorials = () => {
    const targetNumberInput = document.getElementById("targetNumberFactorial");
...
const getTable = () => {
    ...
    const helpMultiplier = document.getElementById("helpMultiplier");
    ...
    if (isNaN(multiplier)) {
        ...
        helpMultiplier.insertAdjacentHTML("beforeend", "<p>Ingresa un número válido.</p>");
...
const validateFizzBuzzQuantity = () => {
    const limitQuantityInput = document.getElementById("fizzBuzzQuantity");
    const helpQuantity = document.getElementById("helpFizzBuzzQuantity");
    ...
    const limitQuantity = parseInt(limitQuantityInput.value);
...
const getFizzBuzz = () => {
    ...
    if (validateFizzBuzzQuantity()) {
        let index;
        for (limitQuantity > 0 ? (index = 1) : (index = -1); limitQuantity > 0 ? index <= limitQuantity : index >= limitQuantity; limitQuantity > 0 ? index++ : index--) {
            fizzBuzzSeries.push(index % 3 === 0 && index % 5 === 0
                ? "Fizz Buzz"
                : index % 3 === 0
                    ? "Fizz"
                    : index % 5 === 0
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const primeInput = primesCard.querySelector("input");
    const primeButton = primesCard.querySelector("button");
    const primeResult = primesCard.querySelector("#isPrimeResult");
    primeInput.addEventListener("input", () => validateInput(primeInput));
    primeButton.addEventListener("click", () => isPrime(primeInput.value, primeResult));
});
const validateInput = (inputNumber) => {
    const helpPrimeNumber = document.querySelector("#helpPrimeNumber");
    const valid = /^[0-9]+$/.test(inputNumber.value) && parseInt(inputNumber.value) >= 2;
    if (valid) {
...
const isPrime = (inputNumber, result) => {
    const number = parseInt(inputNumber);
    result.removeAttribute("hidden");
    if (validateInput(document.getElementById("targetPrimeNumber"))) {
        for (let index = 2; index <= Math.sqrt(number); index++) {
            if (number % index === 0) {
                result.setAttribute("class", "bg-danger-subtle p-2 rounded text-danger");
